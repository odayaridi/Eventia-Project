...
// const cartTicketsRoutes = require('./routes/cartTicketsRoutes');
...
// // This allows your frontend (5173) to talk to your backend (3010)
// app.use(cors({
...
//     delete connectedAttendees[attendeeId];
//   });
// });

// // Make io and connectedAttendees available in req.app
// app.set('io', io);
...
// app.use('/api/cartTickets', cartTicketsRoutes);
...
//   // ===== Messaging: join a private conversation room =====
//   socket.on('join_conversation', ({ conversationId }) => {
//     if (!conversationId) return;
//     socket.join(`conversation_${conversationId}`);
//   });

//   // ===== Messaging: send + persist + broadcast =====
...
io.on('connection', (socket) => {
  ...
  // ===== Messaging: send + persist + broadcast + unread notification =====
  socket.on('send_message', async (payload, callback) => {
    try {
      const { conversationId, senderId, message } = payload || {};
  ...
  // ===== Messaging: mark conversation as read through socket =====
