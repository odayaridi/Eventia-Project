...
//   const [selectedVenue, setSelectedVenue] = useState<VenueChattingItem | null>(null);
...
//       if (selectedVenueId) {
//         const matchedVenue = data.find((item) => item.venueId === selectedVenueId);
//         if (matchedVenue) {
...
//       navigate(`/organizer/chat-room/venue/${data[0].venueId}`, { replace: true });
//     } catch (error) {
//       const msg =
...
//             Communicate directly with venue managers through your organized private conversations.
//           </p>
//         </div>

//         <div className="organizer-chat-layout">
//           <section className="surface-card organizer-chat-main-card">
//             <div className="organizer-chat-toolbar">
//               <button
//                 type="button"
...
//                 <p>Please choose a venue from the right panel to start chatting.</p>
//               </div>
...
//                       <div className="organizer-chat-conversation-subitem">
//                         <UserRound size={14} />
//                         <span>{selectedVenue.managerName}</span>
//                       </div>
//                       <div className="organizer-chat-conversation-subitem">
//                         <Building2 size={14} />
...
//                       <p>Start the conversation with this venue manager.</p>
//                     </div>
//                   ) : (
...
//                                 isMine ? "organizer-chat-message-bubble-mine" : ""
//                               }`}
//                             >
...
//                               <p className="organizer-chat-message-text">{msg.message}</p>
//                             </div>
//                           </div>
//                         );
//                       })}
...
//                     {isSending ? (
//                       <>
//                         <LoaderCircle size={16} className="organizer-chat-spin" />
//                         <span>Sending...</span>
//                       </>
//                     ) : (
//                       <>
...
//               <h3>Venues</h3>
//               <p>Select a venue to open its private chat thread.</p>
//             </div>
...
//                   const isActive =
//                     Number(selectedVenue?.venueId) === Number(venue.venueId);
...
//                     >
//                       <div className="organizer-chat-venue-name">{venue.venueName}</div>
...
//                   const unreadCount = Number(venue.unreadCount || 0);
...
const ChatRoom: React.FC = () => {
  ...
  const loadVenues = async () => {
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : "Failed to load venues.";
      showAlert(msg, "error");
      setVenues([]);
      setSelectedVenue(null);
    } finally {
    ...
    <>
      <div className="page-shell organizer-chat-page-shell">
        <div className="page-header">
          <h1 className="page-title">Chat Room</h1>
          <p className="page-subtitle">
            Communicate directly with venue managers through organized private conversations.
          </p>
        </div>
          ...
          <section className="surface-card organizer-chat-main-card">
            ...
            {!selectedVenue ? (
              <div className="organizer-chat-empty-main">
                ...
                <h3>No venue selected</h3>
                <p>Choose a venue from the right panel to open its private chat.</p>
              </div>
            ) : isLoadingConversation ? (
              <div className="organizer-chat-loading-main">
                <LoaderCircle size={24} className="organizer-chat-spin" />
...
// import AlertSnackbar from "../../components/common/AlertSnackbar";
// import { useAlert } from "../../hooks/useAlert";
// import { useCall } from "../../context/webrtc/CallContext";
...
//   const { open, message, severity, showAlert, handleClose } = useAlert();
//   const { socketRef, socketVersion, startCall, callState } = useCall();
...
//     const handleReceiveMessage = async (incomingMessage: ChatMessageItem) => {
//       const isActive =
//         Number(incomingMessage.conversationId) ===
//         Number(activeConversationIdRef.current);
...
//     const handleUnreadUpdated = async () => {
//       await refreshVenuesUnreadOnly();
//     };

//     socket.on("receive_message", handleReceiveMessage);
//     socket.on("unread_messages_updated", handleUnreadUpdated);

//     return () => {
//       socket.off("receive_message", handleReceiveMessage);
//       socket.off("unread_messages_updated", handleUnreadUpdated);
//     };
//   }, [socketVersion, userId, organizerId]);
...
//   const handleSendMessage = async () => {
//     if (!messageInput.trim() || !conversation || !userId || isSending) {
//       return;
//     }
...
//       showAlert("You already have a call in progress.", "error");
//       return;
//     }
...
//       const isCurrentSelectedVenue =
//         selectedVenue && Number(selectedVenue.venueId) === Number(target.venueId);

//       if (!activeConversation || !isCurrentSelectedVenue) {
...
//                     disabled={!conversation || callState.status !== "idle"}
...
//                         onClick={() => handleStartCall(venue)}
//                         disabled={callState.status !== "idle"}
//                         title={`Video call ${venue.managerName}`}
//                       >
//                         <Phone size={15} />
//                       </button>
